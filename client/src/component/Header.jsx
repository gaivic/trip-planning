...
import { getUser, createUser, getRequest } from '../api/users';
...
const Header = ({ user }) => {
    ...
    const getUserReq = async () => {
        ...
        if (fetchedUser.length === 0) {
          ...
          const fetched = await getRequest(createdUser[0]);
          setRequest(fetched);
        }
        else{
          console.log(fetchedUser[0]);
          setUser(fetchedUser[0]);
          const fetched = await getRequest(fetchedUser[0]);
          setRequest(fetched);
        }
    }
        ...
        <div>
            <header className='flex justify-between px-4 py-3  items-center  shadow-sm z-10 border-b-2 border-gray-300'>
                ...
                <div className='flex gap-2 px-4 py-2 border border-gray-300 rounded-full '>
                    ...
                    {openDropdown && <Dropdown openDropdown={openDropdown} setOpenDropdown={setOpenDropdown} setCloseFlagRef={setCloseFlagRef} request={request} User={User}/>}
...
const Dropdown = ({ openDropdown, setOpenDropdown, setCloseFlagRef, request, User }) => {
    ...
    return (
      <div className="absolute w-80 h-96 top-14 right-16" ref={dropdownRef}>
        {<Requests userRequest={request} User={User}/>}
      </div>
    )
}
